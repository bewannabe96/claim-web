...
| Session replay (화면 녹화) | `disable_session_recording: true` |
...
| `<input type=password/email>` 값 | autocapture 기본 마스킹 |
| `<input type=text/tel/hidden>` 값 | autocapture 가 value 미캡처 (form name/type 만) |
| PII element 의 text + attribute | `NO_TRACK_CLASS` / `<NoTrack>` (아래 audit 표) |
...
기본이 `type=text/tel/hidden` 의 value 를 캡처하지 않아 (form name/type 만
첨부) leak risk 없음. session replay enable 시점에는 재검토.

PII 가 새로 추가되는 화면은 위 audit 표에 한 줄 추가 + 적용. 새 input 만들 때
`name` 이 `phone` / `name` / `birth*` / `rrn*` / `code` 면 자동 의심 대상.

### Session replay 도입 시 추가 작업

`disable_session_recording: false` 로 enable 할 때는:
- 위 element 들에 `data-ph-mask` 도 함께 추가 (replay 마스킹은 별개 메커니즘 —
  `no-track.tsx` 가 그때 `NO_TRACK_CLASS` 와 `<NoTrack>` 의 의미를 함께 확장)
- 결제 페이지 (`partner/(dashboard)/credits/topup/*`) 전체를
  `data-ph-mask-everything` 으로 감싸기 — PortOne iframe 밖의 결제 요약/이력도 포함
- replay 보관 기간 (free 1개월) 와 5K/월 한도 검토
- "로깅 이벤트 인벤토리" 의 "의도적으로 로깅 안 되는 것" 표에서 session replay
  행 제거 + replay event 추가
