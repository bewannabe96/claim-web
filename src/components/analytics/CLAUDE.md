...
| 가입자 결과 | `features/plan-proposals/ui/proposal-result-view.tsx` + `contact-cta-button.tsx` | PartnerNoteBubble 호출지점 wrap + attribution 섹션 (파트너명·경력·trustMetric·avatar) — proposal-result-view 가 책임. 하단 CTA button 안 파트너명 span (button click 자체는 추적 유지) — contact-cta-button 가 책임. 어드민 preview (`PreviewResultView`) 도 같은 컴포넌트들 재사용. |
