...
요청 → [① 루트 middleware.ts] → [② (dashboard)/layout.tsx requirePartnerSession] → 페이지
                ↓                            ↓
        Supabase 세션 optimistic 검사   claim.partner row 존재
...
```ts
await requirePartnerSession();  // dal.ts — user + partner extension row 존재 검증
```
...
- `claim.partner` — partner extension. PK = `user.id` 공유. row 존재 = 로그인 자격. `active` 는 매칭 풀(후보 산출) 노출 토글 — 로그인과 무관.
...
| `ALIGO_KEY`, `ALIGO_USER_ID`, `ALIGO_SENDER`, `ALIGO_TEST_MODE` | 휴대폰 OTP SMS + 알림톡 발신 자격 — 마케팅과 공유 (`server/aligo.ts`). |
| `ALIGO_KAKAO_SENDER_KEY` | (운영 필수) 알림톡 발신프로파일 키 — 신규 배정 / 연락 요청 알림톡. 미설정 시 `sendAlimtalk` throw. dev 는 `ALIGO_TEST_MODE=Y` 라 비워둬도 동작. |
...
### 매칭 풀에서 제외 (가입 완료된 partner)

```sql
UPDATE claim.partner SET active=false WHERE id='<user.id>';
```
후보 산출 (`findAssignmentCandidates`) 에서 즉시 제외 — 신규 요청에 후보로 노출되지 않음. 로그인/대시보드 접근은 영향 없음 (`active` 는 매칭 풀 노출만 담당).
