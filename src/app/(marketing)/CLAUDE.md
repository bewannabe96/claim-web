...
가입자는 계정/세션이 없다. 권한 boundary 는 다음 두 가지로만:

1. **`request.id` (16자 nanoid, ~96 bit)** — wizard 단계 (`candidates`/`confirm`/`dispatched`) 진입 토큰. 액션 진입부에서 `status` (`selecting`/`confirming`/`dispatched`) 검증으로 단계 우회 차단.
