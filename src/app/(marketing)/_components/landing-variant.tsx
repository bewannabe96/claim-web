...
import { VariantV3 } from "./variants/v3";
 ...
 *   2. [variants/v3/index.tsx](variants/v3/index.tsx) 같은 파일 생성 +
 *      `VariantV3` export
 ...
 *
 * **현재 v1 / v2 비활성, v3 단독 운영** — VariantV1 / VariantV2 컴포넌트는
 * 보존되어 있지만 dispatcher 에서 빠져 있다. 재활성 절차는
 * [src/lib/lp-variant.ts](../../../lib/lp-variant.ts) 의 `VARIANT_IDS` 코멘트
 * 참조.
 */
type Props = {
  variant: VariantId;
...
export function LandingVariant({ variant, googleAdsConversionTarget }: Props) {
  switch (variant) {
    case "v3":
      return (
        <VariantV3 googleAdsConversionTarget={googleAdsConversionTarget} />
      );
    default: {
      // Exhaustive 체크 — VARIANT_IDS 늘리고 case 안 늘리면 type error.
      const _exhaustive: never = variant;
      void _exhaustive;
      // 런타임 안전망: 알 수 없는 변형은 현재 단독 운영 중인 v3 로 fallback.
      return (
        <VariantV3 googleAdsConversionTarget={googleAdsConversionTarget} />
      );
    }
