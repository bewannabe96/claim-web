...
/**
 * 랜딩 변형 V1 (control) — 현재 운영 중인 랜딩.
 *
 * 구조: Hero(스크롤 반응형 제품 화면) → How it works → Footer → StickyBottomCta.
 *
 * 요청서 작성 CTA 는 Hero 데모의 zone 3 에 임팩트 있게 한 번만 둔다 (별도
 * Closing CTA 섹션은 중복이라 제거).
 *
 * 진입 즉시 인터랙티브 비교 화면을 보여준다. 헤드라인 "설계사가 제안하고 ·
 * AI가 비교하고 · 당신은 선택합니다" 는 단 한 곳(Hero)에만 쓰고, 스크롤하면
 * 상단에 고정돼 데모의 현재 zone 에 맞춰 구절이 하이라이트된다 — 각 구역에 같은
 * 문구를 반복하지 않고 헤드라인이 곧 진행 가이드.
 *
 * 480px 모바일 컨테이너 안 세로 long-scroll. 색은 흑백/그레이만 (DESIGN.md
 * 모노크롬 시스템). 섹션은 스크롤에 맞춰 등장(.reveal).
 *
 * # 변형 디렉토리 규칙
 *
 * 이 변형이 쓰는 sub-component (HeroExperience / StickyBottomCta / 그 안의
 * landing-cta-button / proposal-comparison-demo / demo-proposals) 는 아직 라우트의
 * `_components/`, `_lib/` 에 그대로 남아 있다. v2/v3 가 추가되어 분기 의도가
 * 명확해지는 시점에:
 *   - 다른 변형이 import 하지 않는다 → 이 디렉토리 (`variants/v1/`) 로 이전
 *   - 두 개 이상 변형이 공유한다 → `_components/shared/` 로 승격
 * 둘 중 하나로 정리. v1 단독 PR 에선 이전 비용 회피 + 기존 import 경로 유지.
 */
export function VariantV1({
