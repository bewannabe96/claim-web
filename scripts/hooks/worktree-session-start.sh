...
# 아니면 백그라운드로 풀 부트스트랩 (compose up + migration + seed). 로그는 .claude/workspace-bootstrap.log.
...
mkdir -p .claude
nohup bash scripts/setup-workspace-env.sh >.claude/workspace-bootstrap.log 2>&1 &
disown
