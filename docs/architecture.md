...
User 에는 role discriminator 컬럼이 없음 — 역할은 partner / admin extension row 존재 자체로 판정 (admin 은 추가로 `active=true` 필요, partner 는 존재만으로 충분). 한 사용자가 partner + admin 둘 다 가질 수 있고, 각 require\*Session() 은 자신이 필요로 하는 extension 만 확인.
...
3. **권한 (authz)** — 역할별 `getOptional*Session()` 가 해당 extension row (`claim.admin` / `claim.partner`) 의 존재 확인 — admin 은 추가로 `active=true` 까지 (partner 의 `active` 는 매칭 풀 노출 토글이라 세션 게이트와 무관). 통과해야 세션 발급. `require*Session()` 는 null 시 각 영역 login 페이지로 redirect (admin: `/admin/login`, partner: `/partner/login`). 모든 보호 페이지 layout + 모든 mutation server action 진입부에서 호출.
...
- **Partner — 로그인** (`/partner/login`) — 이미 가입한 partner. Kakao OAuth. `signInWithKakao` action 이 `signInWithOAuth` URL 반환 → Kakao 인증 → `/api/auth/callback` 라우트가 code→session 교환 + user lookup + partner extension 존재 검증 (`active` 무관) → `?next` (middleware 가 미인증 진입 시 보존한 원 경로, 기본 `/partner`, `safeNextPath` 화이트리스트 통과한 것) 로 redirect.
...
**검색 엔진 색인 차단 (프로덕션: admin 만)** — middleware 가 `X-Robots-Tag: noindex, nofollow, noarchive, nosnippet, noimageindex` HTTP 헤더를 모든 `/admin/*` 응답 + 404 응답 + knock 응답에 자동 부착. `src/app/admin/layout.tsx` 의 `metadata.robots` 가 `<meta name="robots">` 로 이중 방어. 프로덕션 `robots.txt` ([src/app/robots.ts](../src/app/robots.ts)) 에 `Disallow: /admin` 은 의도적으로 추가하지 않음 — 경로 존재를 노출하는 역효과.

**비프로덕션 전체 크롤링 차단** — `ENV_STAGE` 가 `production`/`prod` 가 아닌 모든 환경(dev / staging / preview / qa …)에서는 **사이트 전체** 크롤링을 막는다. 두 레이어:
- `src/app/robots.ts` — `robots.txt` 를 `User-agent: * / Disallow: /` 로 생성해 크롤(수집) 자체를 차단. `connection()` 으로 빌드타임 prerender 를 막아 요청 시점의 `ENV_STAGE` 를 반영.
- middleware — 매칭하는 모든 페이지 응답에 `X-Robots-Tag` 색인 차단 헤더를 부착해, robots.txt 를 무시하고 수집된 경우의 색인까지 차단.

프로덕션 판정은 `isProductionEnv()` ([src/lib/env-stage.ts](../src/lib/env-stage.ts)) 가 단일 진실 공급원 — prod allowlist (fail-safe): `ENV_STAGE` 미설정/오타 시 자동으로 차단 쪽으로 떨어진다.
