...
- **G3**. 회원 가입 trigger 는 사용자가 가치를 충분히 체감한 후에 온다 (3번째 업로드 또는 풀 진입). 회원가입 conversion 의 lead quality 가 높아진다.
...
|---|---|---|---|---|
| **0. 익명 게스트** | 비로그인. PII 0. | **쿠키 토큰** (HttpOnly, server-issued nanoid) | 외부 제안서 업로드 **최대 2개**. 비교 화면 열람. | 서버 측 익명 workspace row + S3 PDF. 30일 무활동 시 GC. |
...
쿠키 토큰으로 잡고 있던 익명 workspace 는 회원 가입 트랜잭션 안에서 새 `User.id` 로 owner 교체된다. 토큰은 invalidate. 업로드된 제안서 2개와 그 어림 분석 결과는 그대로 회원의 첫 workspace 로 흡수.
...
비회원이 다음 중 하나를 트리거할 때 차단되고 가입 modal 띄움:
- 3번째 업로드 시도
- 파트너 풀 진입 시도 ([풀에서 받기] 클릭)
...
- **rate-limit**: 1 쿠키 = 2 업로드 hard limit. 쿠키 삭제로 우회 방지 위해 IP+UA fingerprint 보조 카운터 (best-effort, v2 launch 후 abuse 보고 시 강화).
...
- 랜딩 → 첫 슬롯 채움 (업로드 1 OR 풀 진입) — **목표 35%** (현 v1 의 wizard 1-step → finalize ~12% 대비)
- 첫 슬롯 → 두 번째 슬롯 추가 — **목표 50%**
- 두 번째 슬롯 → 회원 가입 게이트 트리거 — **목표 30%**
- 회원 가입 게이트 노출 → 가입 완료 — **목표 60%** (이미 가치 체감한 상태이므로 v1 wizard finalize 보다 높을 것)
...
| # | 질문 | 누가 답 | 결정 기한 |
|---|---|---|---|
| Q1 | "어림 분석 정확도 게이지" — 신뢰도 dimming 외에 사용자에게 어떻게 표현할지 (퍼센트? 별점? 텍스트만?) | 디자인 + PM | 디자인 phase |
| Q2 | 풀 수신 batch "영원히 1회" 정책의 후속 — 7일/30일 경과 시 재요청 가능 여부 | PM + 데이터 | v2 launch 후 30일 |
| Q3 | 게스트 GC 30일 정책의 적정성 — UX 데이터 보고 조정 | PM + ops | launch 후 60일 |
| Q4 | 회원 가입 시 카카오 sso 추가 시점 — OTP 만으로 conversion 충분한지 측정 후 결정 | PM | launch 후 60일 |
| Q5 | 어림 분석 fallback 매핑 정확도 — 카테고리 일치만으로 충분한지, 보장기간 등 추가 시그널 도입 필요 | 분석팀 + PM | launch 후 30일 |
| Q5-1 | external_analyzer 가 책임지는 공통 dimension 의 정확한 list — 어떤 dimension 까지 약식 입력으로 신뢰성 있게 산출 가능한가 | 분석팀 + PM | Phase B 설계 |
| Q5-2 | 외부 업로드 슬롯에서 채워지지 않은 dimension 의 비교 UI 처리 — "정보 없음" 으로 dimming 할지, 슬롯을 비교 축에서 제외할지 (per-dimension toggle) | 디자인 + PM | Phase A/B 디자인 |
| Q6 | 어드민 indexing 큐의 SLA 인력 배치 — 일평균 처리량 가정과 staffing | ops | launch 1주 전 |
| Q7 | 외부 업로드 제안서의 "설계사 이름" 식별 — 가입자가 입력하는 free text 의 운영 정책 (PII? 마스킹?) | 보안 + PM | 설계 phase |
...
- `GuestWorkspace` + 쿠키 토큰 + 2개 한도
- 회원 가입 게이트 + 익명 → 회원 승계
- 랜딩 default 가 `/compare` 빈 상태로 전환
- 목표: PMF entry 가설 측정 (§9.1 의 funnel)
