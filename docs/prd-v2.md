...
- **2번째 업로드 시도** — 첫 슬롯은 시승으로 허용, 두 번째 슬롯 추가 의도가 곧 "비교하고 싶다" 는 명시 시그널이므로 이 시점이 게이트.
...
- 첫 슬롯 보유 → 두 번째 슬롯 추가 시도 (= 회원 가입 게이트 노출) — **목표 50%**
- 회원 가입 게이트 노출 → 가입 완료 — **목표 60%** (1 슬롯으로 이미 가치 체감, 비교하려는 명시 의도까지 표현한 상태)
