...
# 백그라운드로 pnpm install + Docker 기동 + migration + seed 일괄 수행.
# 명시적으로 한 번에 끝내려면:
...
| 명령 | 용도 |
|---|---|
| `pnpm db:start` | 첫 진입 시. Docker 기동 + migration 적용 + seed. 멱등. SessionStart hook 이 자동 호출. |
...
| `pnpm db:cleanup-orphans` | 사라진 worktree 의 고아 컨테이너 + 볼륨 일괄 삭제 (확인 없음). |
...
| Hook | 스크립트 | 동작 |
|---|---|---|
| `SessionStart` | [scripts/hooks/session-start-db.sh](../scripts/hooks/session-start-db.sh) | 컨테이너 running 이면 no-op. 없거나 정지면 백그라운드로 `pnpm db:start` 풀 부트스트랩 (compose up + migration + seed). 로그 `.claude/db-bootstrap.log`. |
| `SessionEnd` | [scripts/hooks/session-end-db.sh](../scripts/hooks/session-end-db.sh) | worktree 한정 `docker compose down -v` — 컨테이너 + 볼륨 완전 삭제. |
...
**부트스트랩 race 주의**: SessionStart 직후 30~60초 안에 사용자/AI 가 DB 를 치면 prisma 명령이 실패할 수 있음. `tail -f .claude/db-bootstrap.log` 로 진행 확인하거나, foreground `pnpm db:start` 재실행 후 재시도.
...
`.claude/worktrees/` 에 디렉토리가 없는 모든 `claim_*_postgres` 컨테이너 + `_pgdata` 볼륨 즉시 삭제. 확인 프롬프트 없음. 메인 리포 basename 은 keep 룰에 포함되어 보호.
